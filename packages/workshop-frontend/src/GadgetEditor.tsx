...
function formatHeaderCost(cost: number) {
  if (cost === 0) return formatNumber(0, { style: 'currency', currency: 'USD', maximumFractionDigits: 0 })
  if (cost < 0.01) return `<${formatNumber(0.01, { style: 'currency', currency: 'USD', minimumFractionDigits: 2 })}`
  return formatNumber(cost, { style: 'currency', currency: 'USD', minimumFractionDigits: 2, maximumFractionDigits: 2 })
}
...
function rightTabs(output: BlueprintOutput | undefined, codeLabel: string, connectionsLabel: string): { value: RightTab; label: string }[] {
  return [
    { value: 'app', label: formatOf(output).noun },
    { value: 'code', label: codeLabel },
    { value: 'connections', label: connectionsLabel },
  ]
}

const ACTIVITY_TAB_VALUES: ActivityView[] = ['review', 'auto', 'history']
...
function NoGadgetPlaceholder({ height }: { height: string }) {
  const { t: translate } = useTranslation('workspaces')
  return (
...
export default function GadgetEditor() {
  ...
  const {
    ...
    onInvalidShareKey: () => {
      toasts.add({ title: translate('detail.invalidShare'), variant: 'error' })
    },
  })
  ...
  const handleRenameWorkpiece = useCallback(async (workpieceId: WorkpieceId, title: string) => {
    ...
    try {
      ...
      toasts.add({ title: translate('detail.renameOutputFailed'), variant: 'error' })
    } finally {
  ...
  const handleSaveTitle = async () => {
    ...
    } catch { toasts.add({ title: translate('detail.updateTitleFailed'), variant: 'error' }) }
  }
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      toasts.add({ title: translate('detail.deleteFailed'), variant: 'error' })
      setIsDeleting(false)
      ...
      <div className="min-h-screen flex items-center justify-center flex-col gap-4 bg-kumo-base">
        ...
        <div className="flex items-center gap-2">
          <WorkshopButton tone="secondary" onClick={handleGoToWorkspaces}>
            {translate('detail.goToWorkspaces')}
          </WorkshopButton>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-kumo-base">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-kumo-subtle">{translate('detail.loading')}</p>
        </div>
        {observerConfig && (
          <ObserverConfigModal
    ...
    <div className="flex flex-col h-screen overflow-hidden bg-kumo-base relative">
      ...
      >
        ...
        <div className="flex items-center gap-2 min-w-0">
          <Link
            to="/"
            aria-label={translate('detail.home')}
          ...
          {isEditingTitle ? (
            <div className="flex items-center gap-1">
              ...
              <WorkshopIconButton
                ...
                aria-label={translate('detail.saveTitle')}
              >
                <Check size={14} />
              </WorkshopIconButton>
              <WorkshopIconButton
                ...
                className="!h-7 !w-7"
                aria-label={translate('detail.cancelTitle')}
              >
                <X size={14} />
              </WorkshopIconButton>
            </div>
            ...
            <div className="flex items-center gap-1 min-w-0">
              ...
              <WorkshopIconButton
                ...
                title={translate('detail.renameTitle')}
                aria-label={translate('detail.renameTitle')}
              >
                <Pencil size={16} />
              </WorkshopIconButton>
            </div>
          ...
          {metadata.owner && (
            <span className="text-xs text-kumo-inactive flex-shrink-0">
              {translate('detail.by', { name: metadata.owner.name })}
            </span>
          )}
        </div>
        ...
        <div className="flex items-center gap-1 flex-shrink-0">
          ...
          {connectionLost && (
            <span className="text-xs text-kumo-warning px-2 py-0.5 rounded-full bg-kumo-warning-tint border border-kumo-warning/20">
              {translate('detail.reconnecting')}
            </span>
          )}

          <WorkshopIconButton
            onClick={() => setShareModalOpen(true)}
            title={translate('detail.share')}
            aria-label={translate('detail.share')}
          >
          ...
          <WorkshopIconButton
            ...
            title={translate('detail.blueprints')}
            aria-label={translate('detail.blueprints')}
          >
            <Blueprint size={16} />
          </WorkshopIconButton>
          ...
          {!metadata.owner && (
            <WorkshopIconButton
              ...
              title={translate('detail.delete')}
              aria-label={translate('detail.delete')}
            >
              <Trash size={16} />
            </WorkshopIconButton>
          )}
              ...
              {!layoutModeReady && (
                <div className="absolute inset-0 flex items-center justify-center bg-kumo-base">
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-sm text-kumo-subtle">{translate('detail.loadingConversation')}</p>
                  </div>
                </div>
              )}
            </div>
        ...
        >
          ...
          >
            <div className="flex min-w-0 flex-1 items-center overflow-hidden">
              {paneShowsActivity ? (
                <PaneLabel icon={Pulse} title={translate('detail.activity')} />
              ) : visibleGadgets.length > 1 ? (
                <PaneWorkpieceTabs
                  gadgets={visibleGadgets}
                ...
                <PaneLabel
                  ...
                  badge={selectedGadgetSummary.chatId !== undefined ? translate('detail.draft') : undefined}
                />
              )}
            </div>
            ...
            <div className="flex flex-shrink-0 items-center gap-1.5">
              <div className="flex items-center rounded-lg border border-kumo-line p-0.5">
                {paneShowsActivity
                  ? ACTIVITY_TAB_VALUES.map(value => (
                    <PaneTab
                      key={value}
                      active={activityView === value}
                      label={translate(`detail.tabs.${value}`)}
                      count={value === 'review' ? pendingActionsCount : undefined}
                      onClick={() => setActivityView(value)}
                    />
                  ))
                  : rightTabs(
                      selectedGadgetSummary?.output,
                      translate('detail.tabs.code'),
                      translate('detail.tabs.connections'),
                    ).map(tab => (
                    <PaneTab
                      key={tab.value}
              ...
              {!paneShowsActivity && (
                <GadgetExportMenu
                  gadget={selectedGadgetStub}
                  gadgetTitle={selectedGadgetSummary?.title ?? translate('detail.fallbackOutput')}
              ...
              {!paneShowsActivity && (
                <WorkshopIconButton
                  aria-label={translate('detail.enterFullScreen')}
                  title={activeTab === 'app' && !previewMode
                    ? translate('detail.fullScreen')
                    : translate('detail.fullScreenUnavailable', { view: formatOf(selectedGadgetSummary?.output).noun })}
              ...
              <WorkshopIconButton
                aria-label={paneShowsActivity ? translate('detail.closeActivity') : translate('detail.closePane')}
                title={translate('detail.close')}
          ...
          <div className="flex-1 min-h-0 overflow-hidden">
            ...
            <div
              ...
              aria-label={isGadgetFullscreen ? translate('detail.fullScreenAria') : undefined}
              className={
              ...
              {isGadgetFullscreen && showFullscreenHint && (
                ...
                >
                  <div className="rounded-full border border-kumo-line bg-kumo-base/90 px-4 py-1.5 text-[13px] leading-[18px] text-kumo-default shadow-md backdrop-blur-sm">
                    {translate('detail.fullScreenHint')}
                  </div>
                </div>
              )}
            </div>
      ...
      <DeleteConfirmationDialog
        ...
        title={translate('detail.deleteDialogTitle')}
        description={translate('detail.deleteDialogDescription', { title: metadata.title })}
