...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    const fetchUserInfo = async () => {
      try {
        ...
        if (!cancelled) toasts.add({ title: t('messages.loadFailed'), variant: 'error' })
      } finally {
  ...
  const handleSaveName = async () => {
    if (!nameInput.trim()) {
      toasts.add({ title: t('messages.emptyDisplayName'), variant: 'error' })
      return
    }
    ...
    try {
      ...
      toasts.add({ title: t('messages.displayNameUpdated'), variant: 'success' })
    } catch (err) {
      console.error('Failed to update display name:', err)
      toasts.add({ title: t('messages.displayNameUpdateFailed'), variant: 'error' })
    }
  }
  ...
  const handleCopyId = async () => {
    ...
    try {
      ...
      toasts.add({ title: t('messages.userIdCopied'), variant: 'success' })
    } catch {
      toasts.add({ title: t('messages.copyFailed'), variant: 'error' })
    }
  }

  const handleAvatarUpload = async (file: File) => {
    if (!file.type.startsWith('image/')) {
      toasts.add({ title: t('messages.imageRequired'), variant: 'error' })
      return
    }
    ...
    try {
      ...
      toasts.add({ title: t('messages.avatarUpdated'), variant: 'success' })
    } catch (err) {
      console.error('Failed to upload avatar:', err)
      setLocalAvatarPreview(null)
      toasts.add({ title: t('messages.avatarUpdateFailed'), variant: 'error' })
    } finally {
  ...
  const handleChangePassword = async () => {
    ...
    try {
      ...
      toasts.add({ title: t('messages.passwordChanged'), variant: 'success' })
      setCurrentPassword('')
      setNewPassword('')
      setConfirmPassword('')
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : t('messages.passwordChangeFailed')
