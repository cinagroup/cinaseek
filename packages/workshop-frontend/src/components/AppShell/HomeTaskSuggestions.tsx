...
export default function HomeTaskSuggestions({
  ...
  return (
    <section aria-label={t('suggestions.label')} className="flex flex-col gap-1">
      ...
      <ul className="flex flex-col gap-0.5">
        {visible.map((suggestion) => (
          <SuggestionRow
            ...
            label={t(`suggestions.${suggestion.id}.label`)}
            description={t(`suggestions.${suggestion.id}.description`)}
            onClick={() => onPick(t(`suggestions.${suggestion.id}.prompt`))}
          />
        ))}
      </ul>
    </section>
