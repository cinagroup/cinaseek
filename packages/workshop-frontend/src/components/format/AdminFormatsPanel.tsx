...
export default function AdminFormatsPanel({
  ...
  const mutate = async (op: () => Promise<void>) => {
    ...
    try {
      ...
      toasts.add({ title: translate('formats.updateFailed'), variant: 'error' })
    } finally {
      setBusy(false)
    }
  }
  ...
  return (
    <div className="rounded-xl border border-kumo-line bg-kumo-elevated p-6">
      <h2 className="mb-1 text-lg font-semibold text-kumo-strong">
        {translate('formats.title')}
      </h2>
      <p className="mb-5 text-sm text-kumo-subtle">{translate('formats.description')}</p>
            ...
            >
              ...
              <span className="min-w-0">
                <span className="block truncate text-[13px] text-kumo-default">
                  {candidate.title || translate('formats.untitled')}
                </span>
                <span className="block truncate text-[11px] text-kumo-inactive">
                  {candidate.declared
                    ? translate('formats.produces', { plural: candidate.declared.plural })
                    : translate('formats.noDeclared')}
                </span>
              </span>
            </DropdownMenu.Item>
          ))}
  ...
  return (
    <div className="mb-5 rounded-lg border border-dashed border-kumo-line bg-kumo-tint/40 p-4">
      <p className="mb-2.5 text-[11px] font-medium uppercase tracking-[0.06em] text-kumo-inactive">
        {translate('formats.previewTitle')}
      </p>
      {formats.length === 0 ? (
        <p className="text-[13px] italic text-kumo-inactive">
          {translate('formats.previewEmpty')}
        </p>
      ) : (
        <div className="flex flex-wrap items-center gap-2">
          {formats.map((format) => (
            ...
            >
              ...
              {translate('formats.newFormat', { noun: format.output!.noun })}
            </span>
          ))}
        </div>
      )}
      <p className="mt-2.5 text-[12px] leading-4 text-kumo-subtle">
        {translate('formats.previewDescription')}
      </p>
    </div>
  )
}
...
function EmptyState() {
  ...
  return (
    <div className="mb-5 rounded-lg border border-kumo-line bg-kumo-base px-4 py-5 text-center">
      <p className="text-sm font-medium text-kumo-default">{translate('formats.emptyTitle')}</p>
      <p className="mx-auto mt-1 max-w-md text-[13px] leading-[18px] text-kumo-subtle">
        {translate('formats.emptyDescription')}
      </p>
    </div>
  )
}
  ...
  return (
    <div className="group rounded-lg border border-kumo-line bg-kumo-base">
      <div className="flex items-center gap-3 p-3">
        ...
        >
          {format.missing ? (
            <span
              ...
              title={translate('formats.missingTitle')}
            >
              <Warning size={16} />
            </span>
          ) : (
          ...
          <span className="min-w-0 flex-1">
            <span className="flex items-center gap-2">
              ...
              {format.bundled && <Badge>{translate('formats.bundled')}</Badge>}
              {!format.enabled && !format.missing && <Badge>{translate('formats.off')}</Badge>}
              {needsNaming && <Badge tone="warn">{translate('formats.needsName')}</Badge>}
            </span>
            <span className="mt-0.5 block truncate text-xs text-kumo-subtle">
              {format.missing
                ? translate('formats.missingDescription')
                : needsNaming
                ? translate('formats.undeclaredDescription')
                : translate('formats.shownUnder', {
                    blueprint: format.blueprintTitle,
        ...
        >
          <IconButton
            label={translate('formats.moveUp')}
          ...
          <IconButton
            label={translate('formats.moveDown')}
      ...
      {open && (
        <div className="flex flex-col gap-4 border-t border-kumo-line px-3 py-4">
          {format.missing ? (
            <p className="text-[13px] text-kumo-subtle">{translate('formats.missingDetail')}</p>
          ) : (
            <>
              <Fieldset
                title={translate('formats.presentationTitle')}
                detail={translate(
                  format.bundled
                    ? 'formats.presentationDetailBundled'
                    : 'formats.presentationDetail',
                )}
              >
                <div className="flex items-center gap-4">
                  <div className="grid flex-1 gap-2 sm:grid-cols-[auto_1fr_1fr]">
                    ...
                    />
                    <OverrideField
                      label={translate('formats.plural')}
                  ...
                  <figure className="hidden shrink-0 flex-col items-center gap-1.5 sm:flex">
                    ...
                    <figcaption className="text-[10px] uppercase tracking-[0.06em] text-kumo-inactive">
                      {translate('formats.outputsPreview')}
                    </figcaption>
                  </figure>
                </div>
              </Fieldset>
              ...
              <Fieldset
                title={translate('formats.agentTitle')}
                detail={translate('formats.agentDetail')}
              >
                <OverrideField
                  label={translate('formats.hint')}
                  placeholder={translate('formats.hintPlaceholder')}
                  ...
                  <p className="mt-2 flex items-start gap-1.5 rounded-md bg-kumo-tint/60 px-2.5 py-2 font-mono text-[11px] leading-4 text-kumo-subtle">
                    ...
                    <span className="min-w-0">
                      <span className="block">
                        {translate('formats.catalogEntry', {
                          noun: format.output.noun,
                          hint: format.agentHint ? ` -- ${format.agentHint}` : '',
                        })}
                      </span>
              ...
              <div className="flex items-end justify-between gap-4 border-t border-kumo-line pt-3">
                <p className="text-[12px] leading-4 text-kumo-subtle">
                  {translate(
                    format.enabled ? 'formats.enabledDetail' : 'formats.disabledDetail',
                  )}{' '}
                  {format.bundled ? translate('formats.bundledDetail') : ''}
                </p>
                {!format.bundled && (
                  <Button variant="secondary" disabled={busy} onClick={onRemove}>
                    <Trash size={13} className="mr-1.5" />
                    {translate('formats.stopOffering')}
                  </Button>
                )}
              </div>
            </>
        ...
        <DropdownMenu.Trigger
          render={
            <button
              type="button"
              disabled={disabled}
              aria-label={translate('formats.chooseIcon')}
