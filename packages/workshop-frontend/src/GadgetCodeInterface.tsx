...
export default function GadgetCodeInterface({ overseer, filesRoot, height = '100%', onCodeChange, selectedChatId = null, proposedChanges, draftProposedChanges, streamingProposedChanges, streamingActiveFile, isAgentActive, isVisible = true, onHasCodeChange }: GadgetCodeInterfaceProps) {
  const { t: translate } = useTranslation('gadgetCode')
  ...
  const handleFileCreate = (filename: string) => {
    ...
    if (filesMap.has(filename)) {
      toasts.add({ title: translate('fileExists', { filename }), variant: 'error' })
      return
    }
    ...
    setActiveFile(filename)
    toasts.add({ title: translate('fileCreated', { filename }), variant: 'success' })
  }
  ...
  const handleFileDelete = (filename: string) => {
    ...
    if (!filesMap.has(filename)) {
      toasts.add({ title: translate('fileNotFound'), variant: 'error' })
      return
    }
    ...
    toasts.add({ title: translate('fileDeleted', { filename }), variant: 'success' })
  }
  ...
  const handleFileRename = (oldName: string, newName: string) => {
    ...
    if (filesMap.has(newName)) {
      toasts.add({ title: translate('fileExists', { filename: newName }), variant: 'error' })
      return
    }
    ...
    toasts.add({ title: translate('fileRenamed', { oldName, newName }), variant: 'success' })
  }
  ...
  const handleFileDownload = useCallback((filename: string) => {
    ...
    if (!ytext) {
      toasts.add({ title: translateRef.current('downloadFailed', { filename }), variant: 'error' })
      return
    }
  ...
  const activeFileModeLabel = isEditingLocked
    ? translate('reviewingChanges')
    : isDiffMode
      ? translate('editingChanges')
      : translate('editing')
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height, width: '100%' }}>
      {hasUnsavedChanges && (
        <div className="bg-kumo-tint border-b border-kumo-line px-4 py-2 flex items-center gap-2 text-sm text-kumo-warning">
          ...
          <span>{translate('connectionIssue')}</span>
        </div>
      )}
        ...
        <div className="flex flex-col bg-kumo-base" style={{ flex: 1, minWidth: 0 }}>
          {activeFile && (
            <div className="flex h-9 shrink-0 items-center justify-between gap-3 border-b border-kumo-line bg-kumo-base px-3">
              ...
              <WorkshopIconButton
                aria-label={translate('downloadFileNamed', { filename: activeFile })}
                title={translate('downloadFile')}
