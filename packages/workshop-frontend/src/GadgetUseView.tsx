    ...
    <div className="flex flex-col h-screen overflow-hidden bg-kumo-base relative">
      ...
      >
        ...
        <div className="flex items-center gap-2 min-w-0">
          <Link to="/" aria-label={translate('home')} className="flex-shrink-0 hover:opacity-80 transition-opacity">
            <SiteLogo size={22}>
          ...
          {metadata.owner && (
            <span className="text-xs text-kumo-inactive flex-shrink-0">
              {translate('byOwner', { name: metadata.owner.name })}
            </span>
          )}
        </div>
      ...
      <div className="flex-1 min-h-0 overflow-hidden">
        {gadget ? (
          ...
          <div className="flex h-full items-center justify-center px-6 text-center">
            <p className="text-sm text-kumo-subtle">{translate('empty')}</p>
          </div>
        )}
      </div>
    </div>
