...
import { personalSearchShortcut, personalSearchConnectUrl, withPersonalSearchShortcuts } from '../../features/connections/personalSearch'
  ...
  const handleConfirmConnect = async (resourceUrlPatterns?: string[]) => {
    ...
    try {
      if (isTargetAmbient) {
        ...
        const preset = personalSearchShortcut(vendorId)
        const { url } = await authenticatedApi.connectAccount(preset ? 'mcp' : vendorId, resourceUrlPatterns)
        window.open(preset ? personalSearchConnectUrl(url, preset) : url, '_blank', 'noopener,noreferrer')
      }
      handleCloseModal()
    } catch (err) {
