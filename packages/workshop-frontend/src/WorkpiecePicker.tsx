...
export default function WorkpiecePicker({
  ...
  const { t } = useTranslation('workpiecePicker')
      ...
      <div className="flex flex-1 flex-col gap-0.5 overflow-y-auto overflow-x-hidden px-1.5 pb-2">
        {gadgets.map(gadget => {
          ...
          return (
            ...
            >
              <Tooltip content={`${gadget.title}${!expanded && isPending ? ` (${t('draft')})` : ''}${hasHook ? ` · ${t('hooksEnabled')}` : ''}`} asChild>
                <button
                  type="button"
              ...
              {expanded && (
                <WorkshopIconButton
                  ...
                  title={t('rename')}
                  aria-label={t('renameNamed', { title: gadget.title })}
                >
        ...
        <Tooltip content={t('viewActivity')} asChild>
          <button
            type="button"
          ...
          >
            ...
            {expanded && <span className="min-w-0 flex-1 truncate">{t('viewActivity')}</span>}
            {expanded ? (
