...
export default function WorkpiecePicker({
  ...
  const { t: translate } = useTranslation('workpiecePicker')
  ...
  return (
    ...
    >
      ...
      >
        {expanded && (
          <span className="text-[11px] font-medium uppercase tracking-[0.06em]">{translate('outputs')}</span>
        )}
      ...
      <div className="flex flex-1 flex-col gap-0.5 overflow-y-auto overflow-x-hidden px-1.5 pb-2">
        {gadgets.map(gadget => {
            ...
            return (
              <div key={gadget.id} className="flex items-center gap-1 py-0.5">
                ...
                <WorkshopIconButton
                  ...
                  className="!h-6 !w-6"
                  aria-label={translate('cancelRename')}
                >
                  <X size={13} />
                </WorkshopIconButton>
              </div>
          ...
          return (
            ...
            >
              <Tooltip content={`${gadget.title}${!expanded && isPending ? ` (${translate('draft')})` : ''}${hasHook ? ` · ${translate('hooksEnabled')}` : ''}`} asChild>
                <button
                  type="button"
                ...
                >
                  ...
                  {hasHook && (
                    <span
                      role="img"
                      aria-label={translate('hooksEnabled')}
                      className={expanded
              ...
              {expanded && (
                <WorkshopIconButton
                  ...
                  aria-label={translate('renameNamed', { title: gadget.title })}
                >
        ...
        <Tooltip content={translate('viewActivity')} asChild>
          <button
            type="button"
          ...
          >
            ...
            {expanded && <span className="min-w-0 flex-1 truncate">{translate('viewActivity')}</span>}
            {expanded ? (
