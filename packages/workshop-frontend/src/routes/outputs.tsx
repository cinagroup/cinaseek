...
function OutputCard({
  ...
  const { t } = useTranslation('outputs')
  const subtitle = [output.workspaceTitle || t('untitledWorkspace')]
  if (output.owner) subtitle.push(t('sharedBy', { name: output.owner.name }))
  subtitle.push(t('workspaceActive', { time: formatRelativeTime(output.lastActive) }))
  return (
    <div
      role="button"
      tabIndex={0}
    ...
    >
      ...
      <div className="hidden shrink-0 items-center gap-6 text-xs text-kumo-inactive lg:flex">
        ...
        <span className="flex w-40 items-center justify-end gap-1 whitespace-nowrap">
          <Clock size={10} />
          {t('workspaceActive', { time: formatRelativeTime(output.lastActive) })}
        </span>
      </div>
      <OutputMenu onOpen={onOpen} onOpenWorkspace={onOpenWorkspace}
                  onRename={onRename} onRemove={onRemove} />
